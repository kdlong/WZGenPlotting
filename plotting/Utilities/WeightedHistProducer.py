...
#        return num
    def produce(self, hist_name, branch_name, cut_string="", append=False): 
        proof = ROOT.gProof
        cut_string = ''.join([self.weight_branch, "*(" + cut_string + ")" if cut_string != "" else ""])
        proof_path = hist_name + "-gen"
        hist_exp = hist_name + "(100, 0, 1000)"
        proof.DrawSelect(proof_path, ">>".join([branch_name, hist_name]), cut_string, "goff")
